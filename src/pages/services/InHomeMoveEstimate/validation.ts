...
export const validateMoveEstimate = (
  ...
  // }
  // if (values.faxPhone) {
  //   errors.faxPhone = "Fax phone is required";
  // }
  if (!values.homePhone) {
  errors.homePhone = "Home phone is required";
...
if (values.cellPhone && !PHONE_REGEX.test(values.cellPhone.replace(/\D/g, ""))) {
  errors.cellPhone = "Enter valid 10 digit phone number";
}

if (values.workPhone && !PHONE_REGEX.test(values.workPhone.replace(/\D/g, ""))) {
  errors.workPhone = "Enter valid 10 digit phone number";
}

if (values.faxPhone && !PHONE_REGEX.test(values.faxPhone.replace(/\D/g, ""))) {
